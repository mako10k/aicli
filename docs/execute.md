...
- `SCRIPT` の許可形:
  ...
  - `s/RE/REPL/` / `s/RE/REPL/g` / `s/RE/REPL/p` / `s/RE/REPL/gp`
- 意味:
  - `p`: 範囲内行を出力
  - `d`: 範囲内行を除外して出力
...
- 正規表現は **POSIX の通常正規表現（BRE相当）** を `regex.h`（`regcomp/regexec`）で評価します。
- 区切りは `/` のみ（`s#...#...#` のような別デリミタは不可）。
- `RE` と `REPL` 内の `/` はエスケープできません（DSL 側の都合で連続トークン化はできるが、ここでは未対応）。
- `REPL` では後方参照（`\1` 等）や `&` 展開は未対応（リテラル置換）。
- `-n` のため、`p` フラグ無しでは出力しません（`s///p` または `s///gp` のみ出力あり）。
- `s///p` の `p` は「置換が発生した行のみ」出力します（`-n` と組み合わせた通常の `sed` と同じ挙動）。

注意: `sed` は安全モデル維持のため "最小サブセット" のみ対応です。
